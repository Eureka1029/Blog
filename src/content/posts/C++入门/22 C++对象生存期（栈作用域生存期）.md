...
draft: false
---
C++的对象生存期（栈作用域生存期） 可复习<a href="/posts/C++入门/17 创建并初始化C++对象" target="_blank" rel="noopener noreferrer">17 创建并初始化C++对象</a>
...
> 合法吗？ 不合法，因为我们没有在堆上分配它，因为并没有使用`new`。我们只在栈上声明它，当我们返回一个指向它的指针时，它会返回一个指向栈内存的指针，而这个栈内存会在我们离开作用域的时候清除。
...
如果你想写一个这样的函数，你基本上有两个选择。 你可以让这个在堆上分配数组，从而确保了它的生存期会一直存在。
```cpp
int* CreateArray()
{
...
> 我们可以利用栈上的变量自动销毁这个特性,帮助我们自动销毁堆上的变量.
...
// Destroyed Entity
```
> 用一个栈上的变量,包裹了一个堆上的变量,在调用析构函数的时候释放堆上的内存

这个ScopedPtr类是在栈上被分配的，意味着如果e被删除了，在析构函数中，会delete这个被包装的Entity指针。即使我们用new来做堆分配，也完成了销毁。
这就是一个smart pointer：unique_ptr做的最基本的事、
